// rust provides no implicit type conversion (coersion) between
...
fn main() {
    ...
    // Error! There are limitation on conversion rules.
    ...
    // Under the hood, the first 8 least significant (LSB) are kept,
    // while the rest towards the most significant bit (MSB) 
    // get truncated
    ...
    // first casting to the corresonding unsigned type. If the most
    ...
    // exceeds the upper bound or is less than the lower bound, 
    ...
    // with unsafe methods, howewer the results might overflow and
