...
// (allocated on the heap) by creating a `Box<T>`. A box is a smart pointer
...
// the heap is freed
//
// Boxed values can be dereferenced using `*` operator; this removes one
...
fn main() {
    ...
    println!("Boxed point occupoes {} bytes on the stack",
